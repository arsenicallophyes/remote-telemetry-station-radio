...
"""
Patched version of the adafruit_rfm9x module. 
Spi python file feteched from: 
...
try:
    from lib.adafruit_bus_device import spi_device as spidev
except ImportError:
    from adafruit_bus_device import spi_device as spidev

from micropython import const


try:
    import supervisor # pyright: ignore[reportMissingModuleSource]
    HAS_SUPERVISOR = not hasattr(supervisor, "ticks_ms")
except ImportError:
    pass
...
class RFM9x:
    ...
    low_datarate_optimize = bool(_RegisterBits(_RH_RF95_REG_26_MODEM_CONFIG3, offset=3, bits=1))
    ...
    def _read_into(self, address: int, buf: WriteableBuffer, length: "Optional[int]" = None) -> None:
        ...
        with self._device as device:
            ...
            # value (read).
            device.write(self._BUFFER, end=1) #type: ignore
            device.readinto(buf, end=length) #type: ignore
    ...
    def _write_from(self, address: int, buf: ReadableBuffer, length: "Optional[int]" = None) -> None:
        ...
        with self._device as device:
            ...
            # indicate a write.
            device.write(self._BUFFER, end=1) #type: ignore
            device.write(buf, end=length) #type: ignore
    ...
    def _write_u8(self, address: int, val: int) -> None:
        ...
        with self._device as device:
            ...
            device.write(self._BUFFER, end=2) #type: ignore
    ...
    def coding_rate(self) -> "Literal[5, 6, 7, 8]":
        ...
        denominator = cr_id + 4
        assert denominator in {5, 6, 7, 8}
        return denominator # type: ignore
    ...
    def enable_crc(self, val: bool) -> None:
        # "Optional"ly enable CRC checking on incoming packets.
        if val:
            self._write_u8(
                _RH_RF95_REG_1E_MODEM_CONFIG2,
    ...
    def tx_done(self) -> bool:
        """Transmit status"""
        return bool((self._read_u8(_RH_RF95_REG_12_IRQ_FLAGS) & 0x8) >> 3)

    def rx_done(self) -> bool:
        """Receive status"""
        return bool((self._read_u8(_RH_RF95_REG_12_IRQ_FLAGS) & 0x40) >> 6)

    def crc_error(self) -> bool:
        """crc status"""
        return bool((self._read_u8(_RH_RF95_REG_12_IRQ_FLAGS) & 0x20) >> 5)
    ...
    def send(
        ...
        *,
        ...
        timed_out = False
        if HAS_SUPERVISOR:
            start = supervisor.ticks_ms() # type: ignore
            while not timed_out and not self.tx_done():
                if ticks_diff(supervisor.ticks_ms(), start) >= self.xmit_timeout * 1000: # type: ignore
                    timed_out = True
        else:
        ...
        *,
        ...
        if HAS_SUPERVISOR:
            ...
            while not timed_out and not self.rx_done():
                if ticks_diff(supervisor.ticks_ms(), start) >= timeout * 1000: # type: ignore
                    timed_out = True
        else:
            start = time.monotonic()
            while not timed_out and not self.rx_done():
        ...
        if self.enable_crc and self.crc_error():
            ...
            if fifo_length < 5:
                ...
                # This will never return, if we enter this "else" statement
